...
max_concurrent (int): Maximale Anzahl paralleler Prozesse.
    ...
    for _ in range(num_processes):
        ...
        # Eingabe der Abhängigkeiten mit Anzeigeoption
        dependencies = []
        while True:
            dep_input = input(
                f"Abhängigkeiten von '{name}' (Komma-getrennt, leer lassen für keine, ? für Liste): "
            )
            if dep_input.strip() == "?":
                if processes:
                    print("Bisher definierte Prozesse:", ", ".join(processes.keys()))
                else:
                    print("Es wurden noch keine Prozesse definiert.")
                continue
            elif not dep_input.strip():
                dependencies = []
                break
            else:
                dependencies = [dep.strip() for dep in dep_input.split(",") if dep.strip()]
                break
